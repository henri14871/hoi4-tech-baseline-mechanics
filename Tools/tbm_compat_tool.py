...
def build_profile_dispatch(index: dict) -> str:
    ...
    lines = []
    lines.append("###############################################################################")
    lines.append("# tbm_compat_runtime.txt")
    lines.append("# Generated by Tools/tbm_compat_tool.py — do not edit manually.")
    lines.append("# Re-run 'tbm_compat_tool.py build' to regenerate.")
    lines.append("###############################################################################")
    lines.append("")
    ...
    for slug in PROFILE_ORDER:
        ...
        if not markers:
            continue
        keyword = "if" if first_detect else "else_if"
        first_detect = False
...
def compile_builtin_profiles(index: dict,
                             scripted_effects_dir: Path = SCRIPTED_EFFECTS_DIR,
                             output_root: Path = DEFAULT_OUTPUT_ROOT):
    """Compile compat_generated bundles into baked runtime files under common/scripted_effects/.

    Produces three kinds of output:
      - tbm_compat_generated_{slug}.txt — per-profile tech grant effects (slug-suffixed)
      - tbm_compat_generated_tiers.txt  — all profile-specific tier assignment effects
      - tbm_compat_runtime.txt          — routing wrappers (replaces the hand-authored fallback)
    """
    ...
    if removed:
        print(f"Cleaned {len(removed)} old generated file(s) from common/scripted_effects")

    profiles = [item for item in index.get("generated_bundles", [])
                if item["slug"] in PROFILE_OPTION_KEYS]
    if not profiles:
        print("No compat profiles found in index — skipping compilation")
        return

    # 2. Read the base evaluation file for profiles without custom tier thresholds
    base_eval_text = ""
    if BASE_EVALUATION_PATH.exists():
        base_eval_text = BASE_EVALUATION_PATH.read_text(encoding="utf-8-sig")
    ...
    for item in profiles:
        slug = item["slug"]
        bundle_dir = output_root / slug / "common" / "scripted_effects"
        techlist_path = bundle_dir / "auto_research_techlist.txt"

        if not techlist_path.exists():
            print(f"[WARN] No tech list for {slug}, skipping")
            continue

        # 3a. Compile tech grant effects with slug-suffixed names
        tech_text = techlist_path.read_text(encoding="utf-8")
        tech_text = rename_grant_effects(tech_text, slug)
        tech_text = sanitize_runtime_effect_text(tech_text)
        ...
        compiled_path.write_text(tech_text, encoding="utf-8")
        compiled_count += 1

        # 3b. Build the slug-specific tier assignment effect
        eval_path = bundle_dir / "tbm_evaluation.txt"
        if eval_path.exists():
            eval_text = eval_path.read_text(encoding="utf-8")
        elif base_eval_text:
            eval_text = base_eval_text
        else:
            print(f"[WARN] No evaluation source for {slug}, skipping tier effect")
            continue

        try:
            tier_block = extract_top_level_block(eval_text, "tbm_assign_power_tier")
            tier_block = rename_tier_effect(tier_block, slug)
            tier_blocks.append(tier_block)
        except RuntimeError as exc:
            print(f"[WARN] Could not extract tier block for {slug}: {exc}")

    # 4. Write the combined tiers file
    if tier_blocks:
        tiers_header = (
            "###############################################################################\n"
            "# tbm_compat_generated_tiers.txt\n"
            "# Profile-specific tier assignment effects.\n"
            "# Generated by Tools/tbm_compat_tool.py — do not edit manually.\n"
            "###############################################################################\n\n"
        )
        tiers_path = scripted_effects_dir / "tbm_compat_generated_tiers.txt"
        tiers_path.write_text(tiers_header + "\n\n".join(tier_blocks) + "\n", encoding="utf-8")
        print(f"[OUTPUT] Tiers file: {tiers_path.name} ({len(tier_blocks)} profiles)")

    # 5. Write the dispatch file — replaces tbm_compat_runtime.txt so there are
    #    no duplicate effect definitions (Clausewitz last-loaded-wins by filename).
    dispatch_text = build_profile_dispatch(index)
    dispatch_path = scripted_effects_dir / "tbm_compat_runtime.txt"
    dispatch_path.write_text(dispatch_text, encoding="utf-8")
    print(f"[OUTPUT] Dispatch file: {dispatch_path.name} (replaced)")

    print(f"Compiled {compiled_count} compat profile(s) into common/scripted_effects")
...
def cmd_build(args) -> int:
    ...
    compile_builtin_profiles(index)
