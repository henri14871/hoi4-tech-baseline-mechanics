#!/usr/bin/env python3
"""
Arms Race Mechanics — Unified Compatibility Profile Tool
=========================================================
...
Replaces the previous multi-file toolchain:
  arm_tech_generator.py, generate_big_mod_compat.py,
...
GAME_RULES_PATH = REPO_ROOT / "common" / "game_rules" / "arm_game_rules.txt"
LOCALISATION_PATH = REPO_ROOT / "localisation" / "english" / "arm_l_english.yml"
...
GRANT_ORDER_BY_BRANCH = {
    "land": [
        "arm_grant_land_infantry", "arm_grant_land_support",
        "arm_grant_land_artillery", "arm_grant_land_anti_air",
        "arm_grant_land_anti_tank", "arm_grant_land_motorised",
        "arm_grant_land_mechanised", "arm_grant_land_light_tanks",
        "arm_grant_land_medium_tanks", "arm_grant_land_heavy_tanks",
        "arm_grant_land_modern_tanks", "arm_grant_land_tanks_generic",
    ],
    "air": [
        "arm_grant_air_fighters", "arm_grant_air_cas",
        "arm_grant_air_heavy_fighters", "arm_grant_air_tactical_bombers",
        "arm_grant_air_strategic_bombers", "arm_grant_air_naval_bombers",
        "arm_grant_air_transport",
    ],
    "naval": [
        "arm_grant_naval_submarines", "arm_grant_naval_destroyers",
        "arm_grant_naval_light_cruisers", "arm_grant_naval_heavy_cruisers",
        "arm_grant_naval_battleships", "arm_grant_naval_carriers",
...
def generate_output_files(techs: list, output_dir: Path, mode: str, mod_name: str,
    ...
    lines.append(f"# Re-run arm_compat_tool.py to regenerate.")
    lines.append(f"# {'=' * 67}")
    lines.append("")
...
def write_preset_evaluation(output_path: Path, preset: dict):
    ...
    header = (
        ...
        "# This file is generated by Tools/arm_compat_tool.py\n"
        "##########################################################\n\n"
    )
...
def build_profile_dispatch(index: dict) -> str:
    ...
    lines.append("# Generated by Tools/arm_compat_tool.py")
    lines.append("###############################################################################")
    lines.append("")
...
def compile_builtin_profiles(compat_root: Path = DEFAULT_OUTPUT_ROOT,
    ...
    tier_lines.append("# Generated by Tools/arm_compat_tool.py")
    tier_lines.append("###############################################################################")
    tier_lines.append("")
...
def update_game_rules(index: dict):
    """Regenerate the arm_compat_profile block in arm_game_rules.txt."""
    ...
    # Find the arm_compat_profile block boundaries
    block_start = text.find("arm_compat_profile = {")
    if block_start == -1:
        print("[WARN] arm_compat_profile block not found in game rules")
        return
    ...
    while pos < len(text):
        ...
        print("[WARN] Unbalanced braces in arm_compat_profile block")
        return
    ...
    new_block_lines = [
        "arm_compat_profile = {",
        '\tname = "ARM_RULE_compat_profile"',
        '\tgroup = "ARM_RULES_GROUP"',
        "",
        "\tdefault = {",
        "\t\tname = ARM_COMPAT_AUTO",
        '\t\ttext = "ARM_OPT_COMPAT_AUTO"',
        '\t\tdesc = "ARM_DESC_COMPAT_AUTO"',
        "\t}",
        "\toption = {",
        "\t\tname = ARM_COMPAT_VANILLA",
        '\t\ttext = "ARM_OPT_COMPAT_VANILLA"',
        '\t\tdesc = "ARM_DESC_COMPAT_VANILLA"',
        "\t}",
    ]
    for slug in active_slugs:
        ...
        new_block_lines.extend([
            "\toption = {",
            f"\t\tname = {key}",
            f'\t\ttext = "ARM_OPT_{key.removeprefix("ARM_")}"',
            f'\t\tdesc = "ARM_DESC_{key.removeprefix("ARM_")}"',
...
def update_localisation(index: dict):
    """Regenerate compat profile entries in arm_l_english.yml."""
    ...
    # We replace from the first ARM_OPT_COMPAT_ line (after Auto/Vanilla) to the last ARM_DESC_COMPAT_ line
    first_profile_line = None
    last_profile_line = None
    auto_vanilla_keys = {"ARM_OPT_COMPAT_AUTO", "ARM_DESC_COMPAT_AUTO",
                         "ARM_OPT_COMPAT_VANILLA", "ARM_DESC_COMPAT_VANILLA"}

    for i, line in enumerate(lines):
        stripped = line.strip()
        # Check if this is a profile-specific compat line (not auto/vanilla)
        if (stripped.startswith("ARM_OPT_COMPAT_") or stripped.startswith("ARM_DESC_COMPAT_")):
    ...
    if first_profile_line is None:
        ...
        for i, line in enumerate(lines):
            if "ARM_DESC_COMPAT_VANILLA" in line:
    ...
    for slug in active_slugs:
        key = PROFILE_OPTION_KEYS[slug]
        opt_key = f"ARM_OPT_{key.removeprefix('ARM_')}"
        desc_key = f"ARM_DESC_{key.removeprefix('ARM_')}"
...
def validate_tech_ids(slug: str, bundle_root: Path, hoi4_dir: Path,
    ...
    compiled_path = SCRIPTED_EFFECTS_DIR / f"arm_compat_generated_{slug}.txt"
...
def validate_counter_variables(slug: str) -> list[ValidationIssue]:
    ...
    counters = re.findall(r"arm_grant_counter_(\w+)", text)
    ...
    if wrong:
        ...
        for w in unique_wrong:
            issues.append(ValidationIssue("ERROR", "counter_vars",
                                          f"Wrong counter suffix: arm_grant_counter_{w} (expected _{slug})"))
    return issues
...
def validate_dispatch_linkage(slug: str) -> list[ValidationIssue]:
    ...
    called = set(re.findall(rf"(arm_grant_\w+_{re.escape(slug)})\s*=\s*yes", dispatch_text))

    # Effects defined in compiled file (top-level definitions)
    defined = set(re.findall(rf"^(arm_grant_\w+_{re.escape(slug)})\s*=\s*\{{", compiled_text, re.MULTILINE))
...
def validate_year_slices(slug: str) -> list[ValidationIssue]:
    ...
    year_defs = set(re.findall(rf"^(arm_grant_\w+_y\d{{4}}_{re.escape(slug)})\s*=\s*\{{", text, re.MULTILINE))

    # Find year-slice calls
    year_calls = set(re.findall(rf"(arm_grant_\w+_y\d{{4}}_{re.escape(slug)})\s*=\s*yes", text))
...
def validate_category_flags(slug: str) -> list[ValidationIssue]:
    ...
    text = compiled_path.read_text(encoding="utf-8")
    flags_used = set(re.findall(r"has_country_flag\s*=\s*(arm_cat_\w+)", text))
...
def run_validation(slugs: list[str], hoi4_dir: Path, workshop_root: Path,
    ...
    for slug in slugs:
        ...
        compiled_file = SCRIPTED_EFFECTS_DIR / f"arm_compat_generated_{slug}.txt"
...
def cmd_rebuild(args) -> int:
    ...
    update_game_rules(index)
    update_localisation(index)
    print("\nRebuilt bundled ARM compatibility profiles.")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Arms Race Mechanics — Unified Compatibility Profile Tool"
    )
    ...
    build_p = subparsers.add_parser("build",
                                    help="Compile staging bundles into runtime scripted effects")
    build_p.set_defaults(func=cmd_build)

    # rebuild
    rebuild_p = subparsers.add_parser("rebuild", help="Generate all + build in one command")
